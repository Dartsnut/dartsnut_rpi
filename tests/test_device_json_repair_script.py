...
def test_repairs_missing_boot_device_json_from_serial_only_working_copy(
    tmp_path: Path,
) -> None:
    repo_dir = tmp_path / "repo"
    repo_dir.mkdir()
...
def test_repairs_missing_working_device_json_from_serial_only_boot_copy(
    tmp_path: Path,
) -> None:
    repo_dir = tmp_path / "repo"
    repo_dir.mkdir()
