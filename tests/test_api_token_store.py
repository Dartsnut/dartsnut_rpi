...
def test_invalid_file_returns_empty_token(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path))
    os.makedirs(tmp_path / ".dartsnut", exist_ok=True)
...
def test_token_path_can_be_overridden(tmp_path, monkeypatch):
    custom_path = tmp_path / "custom" / "token.json"
    monkeypatch.setenv("DARTSNUT_API_TOKEN_PATH", str(custom_path))

    api_token_store.preserve_remote_user_token({"token": "abc"})

    assert api_token_store.token_file_path() == str(custom_path)
    assert api_token_store.get_api_token() == "abc"
