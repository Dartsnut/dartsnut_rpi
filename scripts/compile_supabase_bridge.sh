...
WATCHDOG_OUT_BIN="${REPO_ROOT}/watchdog"
WATCHDOG_TMP_OUT_BIN="${REPO_ROOT}/watchdog.new"
...
echo "Building Supabase bridge and watchdog with embedded credentials..."
cd "${BRIDGE_DIR}"
...
cp "${BRIDGE_DIR}/target/release/dartsnut-watchdog" "${WATCHDOG_TMP_OUT_BIN}"
chmod +x "${WATCHDOG_TMP_OUT_BIN}"
mv -f "${WATCHDOG_TMP_OUT_BIN}" "${WATCHDOG_OUT_BIN}"

echo "Done: ${OUT_BIN}"
echo "Done: ${WATCHDOG_OUT_BIN}"
