...
def resolve_app_metadata(app_id: str) -> dict[str, Any]:
    """Resolve app identity, preferring backend metadata over sideload shape.

    Server-downloaded apps use the backend sidecar. Sideloaded and legacy apps
    without a sidecar are classified from their package: a valid conf.json is
    a widget unless its legacy ``type`` explicitly says otherwise; a configless
    app with main.py is a game.
    """
    safe_id = _validate_app_id(app_id)
    backend = read_app_metadata(safe_id)
    if backend:
        return backend
    ...
    conf_path = os.path.join(target_dir, "conf.json")
    ...
    if not isinstance(conf, dict):
        return {}
    legacy_type = conf.get("type")
    app_type = legacy_type if legacy_type in APP_TYPES else "widget"
    return {
        "id": safe_id,
        "type": app_type,
        "version": str(conf.get("version") or ""),
        "name": str(conf.get("name") or safe_id),
        "preview_urls": [],
        "download_url": "",
        "download_md5": "",
        "updated_at": "",
