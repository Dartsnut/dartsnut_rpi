...
def _bluetooth_qr_payload(local_name: str) -> str:
...
def _create_bluetooth_qr_surface(local_name: str) -> Image.Image:
    """Create a scan-friendly connection QR centered on the main surface."""
    qr = qrcode.QRCode(
        version=None,
    ...
    qr.add_data(_bluetooth_qr_payload(local_name))
    qr.make(fit=True)
